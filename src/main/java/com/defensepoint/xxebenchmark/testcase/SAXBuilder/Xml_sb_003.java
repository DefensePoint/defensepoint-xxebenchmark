...
@Component
public class Xml_sb_003 {
    private static final Logger logger = LoggerFactory.getLogger(Xml_sb_003.class);

    //@PostConstruct
    public void parse() {

        logger.info("Xml_sb_003");

        Thread th = new Thread ( new Xml_sb_003_thread() , "Xml_sb_003_thread");
        th.start();
...
class Xml_sb_003_thread implements Runnable {

    private static final Logger logger = LoggerFactory.getLogger(Xml_sb_003_thread.class);

    @Override
    public void run() {
        logger.info("Start thread: " + Thread.currentThread().getName());

        String testId = "xml-sb-" + OSUtil.getOS() + "-" + System.getProperty("java.version") + "-003";
