...
@Component
public class Xml_stf_002 {
    private static final Logger logger = LoggerFactory.getLogger(Xml_stf_002.class);

    //@PostConstruct
    public void parse() {

        logger.info("Xml_stf_002");

        Thread th = new Thread ( new Xml_stf_002_thread() , "Xml_stf_002_thread");
        th.start();
...
class Xml_stf_002_thread implements Runnable {

    private static final Logger logger = LoggerFactory.getLogger(Xml_stf_002_thread.class);

    @Override
    public void run() {
        logger.info("Start thread: " + Thread.currentThread().getName());

        String testId = "xml-stf-" + OSUtil.getOS() + "-" + System.getProperty("java.version") + "-002";
