...
@Component
public class Xml_xif_001 {
    private static final Logger logger = LoggerFactory.getLogger(Xml_xif_001.class);

    @PostConstruct
    public void parse() {

        logger.info("Xml_xif_001");

        Thread th = new Thread ( new Xml_xif_001_thread() , "Xml_xif_001_thread");
        th.start();
...
class Xml_xif_001_thread implements Runnable {

    private static final Logger logger = LoggerFactory.getLogger(Xml_xif_001_thread.class);

    @Override
    public void run() {
        logger.info("Start thread: " + Thread.currentThread().getName());

        String testId = "xml-xif-" + OSUtil.getOS() + "-" + System.getProperty("java.version") + "-001";
